...
const GROUPS: SettingGroup[] = [
  {
    ...
    items: [
      ...
      },
      {
        type: "toggle",
        key: "uiAlwaysShow",
        label: "Always show UI",
        tooltip: "When on, the viewer chrome (source label, row nav, close button, toolbar) stays visible instead of auto-hiding after inactivity.",
        onSave: () => { for (const c of activeComps) { c.revealColumnNav?.(); c.revealRowNav?.(); } },
      },
      {
        type: "slider",
        key: "uiHideDelay",
        label: "UI hide delay",
        tooltip: "How long the viewer chrome stays visible after the last activity before fading out. Ignored when Always show UI is on.",
        min: 200,
        max: 5000,
        step: 100,
